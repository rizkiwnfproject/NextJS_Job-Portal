    ...
    <>
      <div className="bg-slate-900 text-foreground pt-16 pb-11 px-32">
        <div className="flex flex-row items-start justify-between">
          ...
          <div>
            <div className="text-lg font-semibold text-primary-foreground mb-5">
              About
            </div>
          ...
          <div>
            ...
            <div className="mt-10 inline-flex items-center gap-3">
              <Input placeholder="Email Address" className="bg-white py-5" />
              <Button className="py-5">Subscribe</Button>
            </div>
          </div>
        </div>
        <Separator className="mt-20 mb-11" />
        <div className="flex flex-row items-center justify-between">
          ...
          <div className="space-x-3">
            {socialMediaImg.map((item: string, i:number) =>(
                <Image className="inline" key={i} src={item} alt={item} width={32} height={32} />
            ))}
          </div>
        </div>
      </div>
